...
async function refreshGuideCards() {
  ...
  try {
    const snap = await guidesMaisonRef()
      ...
      .get();
    if (snap.empty) {
      el.innerHTML = '<div style="color:var(--text-light);font-size: calc(14px * var(--ui-text-scale));padding:8px 0">Aucune fiche. Ajoutez des infos utiles (Wi-Fi, urgences, voisins...)</div>';
      return;
    }
    el.innerHTML = snap.docs.map(doc => {
      const d = doc.data();
      return `<div style="background:#f8f9fa;border-radius:12px;padding:14px;margin-bottom:10px">
        <div style="font-weight:700;font-size: calc(15px * var(--ui-text-scale));margin-bottom:6px">${d.emoji || '📌'} ${d.title || ''}</div>
        <div style="font-size: calc(14px * var(--ui-text-scale));color:var(--text);white-space:pre-wrap">${d.content || ''}</div>
      </div>`;
    }).join('');
  } catch(e) {
    el.innerHTML = '<div style="color:var(--danger);font-size: calc(14px * var(--ui-text-scale))">Erreur de chargement.</div>';
  }
}
  ...
  document.getElementById('sheet-content').innerHTML = `
    <div class="login-sheet">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px">
        ${GUIDE_EMOJIS.map(e => `<button style="font-size: calc(24px * var(--ui-text-scale));background:none;border:2px solid transparent;border-radius:8px;padding:4px 6px;cursor:pointer" id="gemoji-${e}" onclick="selectGuideEmoji('${e}', this)">${e}</button>`).join('')}
      </div>
      ...
      <div class="input-group">
        <label>Contenu</label>
        <textarea id="guide-content" rows="4" placeholder="Informations utiles..." style="width:100%;padding:10px;border:1px solid var(--border);border-radius:10px;font-family:'DM Sans',sans-serif;font-size: calc(14px * var(--ui-text-scale));resize:none"></textarea>
      </div>
